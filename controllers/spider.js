...
function getParadigm4Relevant(req, res, list, recommendInfo, userID) {
    const recommendInfoData_rele = recommendInfo.data[0] || {}; // 相关资料
    if (recommendInfoData_rele.useId) {
        ...
        const url = `https://tianshu.4paradigm.com/api/v0/recom/recall?sceneID=${recommendInfoData_rele.useId}`;
...
function getHotpotSearch(req, res, list, recommendInfo, userID) {
    ...
    const recommendInfoData_rele = recommendInfo.data[1] || {}; // 相关资料
    if (recommendInfoData_rele.useId) {
        const requestId = Math.random().toString().slice(-10);// requestID是用来标注推荐服务请求的ID，是长度范围在8~18位的随机字符串
        req.body = {
            request: {
