define(function (require, exports, module) {
    ...
    $(function () {
        ...
        function zoom(enlarge, original, scaleojb, bdwrap, docmain, profile) {
            ...
            var fullscreen = 0, origw, origh, maxw, minw = 220, maxwrap, copyhtml = '', csname1 = '', csname2 = '',
                classdec, classadd, a = 0, prevw = 134;
            ...
            function zoomcss(objcss, multicss, hcss) {
                ...
                if (navigator.appName == 'Microsoft Internet Explorer' && parseInt(navigator.appVersion.split(';')[1].replace(/[ ]/g, '').replace('MSIE', '')) < 9) {
                    $(objcss).attr('style', String('zoom:' + multicss));
                }
            }
            ...
            function bigsmall(originalbs, scaleojbbs, origwbs) {
                var multibs = ($(originalbs).width() / origwbs).toFixed(3) - 0.002;
            ...
            origw = $(original).width(); // 初始状态的被全屏内容的宽度
            origh = $(original).height(); // 初始状态的被全屏内容的高度
            classdec = $('.zoom-decrease').attr('class');
            classadd = $('.zoom-add').attr('class');
            // 放大缩小的倍数
            var multi = ($(window).width() / origw).toFixed(3) - 0.002;
            ...
            $(enlarge).on('click', function () {
                ...
                if ($(this).hasClass('reader-fullScreen')) {
                    ...
                    if (navigator.appName == 'Microsoft Internet Explorer' && parseInt(navigator.appVersion.split(';')[1].replace(/[ ]/g, '').replace('MSIE', '')) < 9) {
                        $('.detail-inner').css({ 'zoom': scale }).parent('.detail-con').height($('.detail-inner').height() * scale);
                    }
            ...
            $(window).resize(function () {
                if ($(original).html() == '') {
                    multi = ($(window).width() / origw).toFixed(3) - 0.002;
                    if (multi > 1) {
                        // 调用公共函数
            ...
            $('.zoom-add').click(function () {
                if (fullscreen == 0) {
                    ...
                    $('.doc-main-br').width($(docmain).width());/* 修复bug*/
                }
            });
            // 事件：缩小
