define(function (require, exports, module) {
    ...
    var columns = require('./columns');                 //上报参数配置
    var config = require('./config');                   //参数配置
    ...
    Report.prototype = {
        ...
        client: function () {
            ...
            if (method.getCookie(this.config.COOKIE_CIDE)) {
                ...
                method.setCookieWithExpPath(this.options.cid, cid, 1000 * 60 * 60 * 24 * 30, '/');
            }
        ...
        clickWatch: function () {
            var that = this;
            $(document).delegate('.' + this.config.EVENT_NAME, 'click', function (event) {//动态绑定点击事件
                var cnt = $(this).attr(that.config.CONTENT_NAME);//上报事件类型
                that.setCurrEventCon(cnt);
                if (($.browser.msie && $.browser.version <= 8 && that.mousePosition(event).x >= 0)
                    || (event.originalEvent && event.originalEvent.isTrusted)) {//判断用户点击 、及内容不能为空
                    if (cnt) {
                        that.push([that.config.CONTENT_NAME, cnt]);
                    }
                }
                $(this).trigger('init', event);//委托元素
            })
        },
        ...
        push: function (d, callback) {
            ...
            this.send(1, reportData);//1 点击事件上报
        ...
        handleCookieFlag: function (val) {
            try {
                ...
                if (cookie) {
                    ...
                    method.setCookieWithExpPath(this.config.COOKIE_FLAG, JSON.stringify(parse), 1000 * 60 * 50, '/');
                }
            } catch (e) {
            }
        },
        ...
        setCurrEventCon: function (cnt) {
            method.setCookieWithExpPath('_dpclkcnt', cnt, 5 * 60 * 1000, '/');
        }
    };

    window._dataReport = new Report(window.pageConfig.report);//点击数据全局对象 兼容登录系统埋点
