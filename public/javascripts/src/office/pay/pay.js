define(function (require, exports, moudle) {
    ...
    if (!method.getCookie('cuk')) {
        console.log("未登录用户~~~~");
        $('.notLogin').trigger('click',true);//点击登录并且触发刷新事件
    } else {
        console.log("登录用户~~~~");
    ...
    function fillReportData(orderNo, name, price, type, ptype) {
        ...
        if (ptype == 0) {
            ...
            method.setCookieWithExpPath('vr',JSON.stringify(customData),5*60*1000,'/');
        } else if (ptype == 1) {
            ...
            if (rf&&f){
                ...
                method.setCookieWithExpPath('pr',JSON.stringify(customData),5*60*1000,'/');
            }else if (ptype == 2){
                var rf = method.getCookie('rf');
                if (rf&&f){
                    rf = JSON.parse(rf);
                    ...
                    method.setCookieWithExpPath('fr',JSON.stringify(customData),5*60*1000,'/');
                }
            }
        }
    }
        ...
        $.post("/pay/orderStatus?ts=" + new Date().getTime(), { 'orderNo': orderNo }, function (data, status) {
            if (data && data.code == 0) {
                ...
                if (orderStatus == 0) {//待支付
                    if (count <= 30) {
                        window.setTimeout(function () { payStatus(orderNo) }, 4000);
                    }
