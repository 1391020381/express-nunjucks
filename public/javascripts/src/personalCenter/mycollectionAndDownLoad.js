...
define(function (require) {
        ...
        $.ajax({
            ...
            success: function (res) {
                if (res.code == '0') {
                    ...
                    var mycollectionAndDownLoadTemplate = template.compile(mycollectionAndDownLoad)({ type: 'mycollection', list: list || [], totalSize: res.data.totalSize || 0 });
    ...
    function getBannerbyPosition() { // PC_M_USER_banner
        $.ajax({
            ...
            type: 'POST',
            data: JSON.stringify({pageIds:recommendConfigInfo.mydownloadBanner.pageIds}),
            contentType: 'application/json; charset=utf-8',
            dataType: 'json',
            success: function (res) {
    ...
    function startTaskReceive(taskList) {
        ...
        var htmlTpl = template.compile(receiveCoupon)({ data: data });
    ...
    $(document).on('click', '.personal-center-dialog .file-rates .start', function () {
        ...
        if (isAppraise != 1) { // 未评论  也就是评论
            ...
            $('.evaluation-dialog .evaluation-confirm').css({ background: '#F25125', color: '#FFFFFF' });
        }
    });
