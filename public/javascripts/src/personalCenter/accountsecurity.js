define(function (require) {
        ...
        $.ajax({
            ...
            success: function (res) {
                if (res.code == '0') {
                    ...
                    var accountsecurityTemplate = template.compile(accountsecurity)({ userBindInfo: res.data });
    ...
    $('#dialog-box').on('click', '.authentication-btn', function () { // 身份验证
        //   var checkIdentity
    ...
    $(document).on('click', '.authentication-code', function () { // 获取验证码   在 authentication-code元素上 添加标识   0 获取验证码    1 倒计时   2 重新获取验证码
    ...
    $('#dialog-box').on('click', '.bind-phonenumber-dialog .confirm-binding', function () { // 绑定手机号 换绑手机号 确认事件
    ...
    $('#dialog-box').on('click', '.unbind-account-dialog .unbind-btn', function () {
        ...
        var clientCode = isTHirdAuthorization == 'unbindWechatAuthorization' ?
            'wechat' : isTHirdAuthorization == 'unbindWeiboAuthorization' ? 'weibo' : 'qq';
