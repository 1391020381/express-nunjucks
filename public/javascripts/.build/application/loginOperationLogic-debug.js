define("dist/application/loginOperationLogic-debug", [ "../cmd-lib/jqueryMd5-debug", "../application/api-debug", "./urlConfig-debug", "../application/method-debug", "../common/bindphone-debug", "./api-debug", "./method-debug" ], function(require, exports, module) {
    ...
    //   保存登录方式在 登录数上报时使用
    $(document).on("click", ".login-content .login-type-list .login-type-weixin .weixin-icon", function(e) {
        // 切换到微信登录
    ...
    $(document).on("click", ".login-content .login-type-list .login-type-verificationCode", function(e) {
        // 切换到验证码
        $(".login-content .password-login").hide();
    ...
    $(document).on("click", ".login-content .login-type-list .login-type-password", function(e) {
        // 切换到密码登录
        $(".login-content .weixin-login").hide();
        $(".login-content .verificationCode-login").hide();
    ...
    $(document).on("click", ".login-content  .login-type-list .login-type", function() {
        // 第三方登录
    ...
    $(document).on("click", ".login-content .login-btn", function(e) {
        //  密码和验证码登录
        var logintype = $(this).attr("data-logintype");
        if (logintype == "verificationCode") {
            ...
            var mobile = $(".login-content .verificationCode-login .telphone").val().trim();
        ...
        if (logintype == "password") {
            ...
            var mobile = $(".login-content .password-login .telphone").val().trim();
            if (!method.testPhone(mobile) && nationCode == 86) {
    ...
    // 微信登录
    function getLoginQrcode(temp, fid, isqrRefresh, isTouristLogin, callback) {
