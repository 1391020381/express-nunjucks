define("dist/pay/pay-debug", [ "swiper-debug", "../application/method-debug", "../cmd-lib/util-debug", "./qr-debug", "../cmd-lib/qr/qrcode.min-debug", "../common/baidu-statistics-debug", "../application/urlConfig-debug", "../application/api-debug", "./couponReceive-debug.html", "../application/effect-debug", "../application/checkLogin-debug", "../application/login-debug", "../application/loginOperationLogic-debug", "../cmd-lib/jqueryMd5-debug", "../common/bindphone-debug", "../cmd-lib/myDialog-debug", "../cmd-lib/toast-debug", "../application/iframe/iframe-messenger-debug", "../application/iframe/messenger-debug", "../common/loginType-debug", "../common/associateWords-debug", "../common/coupon/couponOperate-debug", "../common/coupon/template/options-debug.html", "../common/coupon/couponIssue-debug", "../cmd-lib/loading-debug", "../common/coupon/template/couponCard-debug.html" ], function(require, exports, moudle) {
    ...
    function startCouponReceive(couponList, callback) {
        ...
        var htmlTpl = template.compile(couponReceive)({
            data: data
        });
        if (!$("#receive-coupon-box").html()) {
            $("#receive-coupon-box").html(htmlTpl);
            callback && callback();
        }
    }
    // 绑定定时弹窗关闭按钮
    $(document).on("click", ".coupon-dialog-wrap .coupon-dialog-close", function() {
        // 判断如果弹窗出现
        switchCancel = true;
        switchCount = 0;
        if ($("#receive-coupon-box").html()) {
    ...
    // 绑定立即领取按钮回调
    $(document).on("click", ".coupon-dialog-wrap .coupon-dialog-footer", function() {
        var parmas = {
            type: 2,
            source: 1,
            site: 4
    ...
    $(document).on("click", ".pic-pay-code .pay-qrcode-refresh", function() {
        initPage(userInfo);
    });
    ...
    $(".renewal-label").on("change", function() {
