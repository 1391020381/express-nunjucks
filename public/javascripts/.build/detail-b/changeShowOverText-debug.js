define("dist/detail-b/changeShowOverText-debug", [ "./download-debug", "../application/method-debug", "../cmd-lib/myDialog-debug", "../cmd-lib/toast-debug", "../cmd-lib/loading-debug", "../cmd-lib/util-debug", "./common-debug", "../application/api-debug", "../application/urlConfig-debug", "./template/pay_btn_tmp-debug.html", "./template/pay_middle_tmp-debug.html", "./template/pay_header.tmp-debug.html", "../common/loginType-debug", "../application/checkLogin-debug", "../application/login-debug", "../application/iframe/iframe-messenger-debug", "../application/iframe/messenger-debug", "../common/baidu-statistics-debug", "./index-debug", "../application/suspension-debug", "../application/app-debug", "../application/loadSentry-debug", "../application/element-debug", "../application/template-debug", "../application/extend-debug", "../application/effect-debug", "../application/helper-debug", "../application/single-login-debug" ], function(require, exports, module) {
    // 需要判断时候是否要登录 是否点击
    // 试读完毕后, 修改 继续阅读 按钮的文字 而且修改后 事件的逻辑 走下载逻辑
    var downLoad = require("./download-debug").downLoad;
    ...
    function sentEmail() {
        ...
        if (reward.value == "-1") {
        ...
        } else if (reward.unit == 1 && reward.value == "0") {
            // 当天次数用完
            $("#dialog-box").dialog({
                html: $("#reward-error-pop").html()
            }).open();
        } else if (reward.unit == 0 && reward.value == "0") {
            // 一次性用完
            $("#dialog-box").dialog({
            ...
            // 正常弹起
            $("#dialog-box").dialog({
                html: $("#reward-success-pop").html().replace(/\$value/, reward.value)
            }).open();
        }
