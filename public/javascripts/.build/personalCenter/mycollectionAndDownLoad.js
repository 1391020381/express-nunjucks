define("dist/personalCenter/mycollectionAndDownLoad", [ "../application/method", "../application/api", "../application/urlConfig", "./template/mycollectionAndDownLoad.html", "./template/simplePagination.html", "./dialog", "../application/effect", "../application/checkLogin", "../application/login", "../application/loginOperationLogic", "../cmd-lib/jqueryMd5", "../common/bindphone", "../cmd-lib/myDialog", "../cmd-lib/toast", "../application/iframe/iframe-messenger", "../application/iframe/messenger", "../common/baidu-statistics", "../common/loginType", "../common/associateWords", "./home", "swiper", "../common/recommendConfigInfo", "../common/template/swiper_tmp.html", "./template/homeRecentlySee.html", "./template/vipPrivilegeList.html", "./template/receiveCoupon.html", "./template/commentDialogContent.html" ], function(require) {
        ...
        $.ajax({
            ...
            success: function(res) {
                if (res.code == "0") {
                    ...
                    var mycollectionAndDownLoadTemplate = template.compile(mycollectionAndDownLoad)({
                        type: "mydownloads",
                        list: list || [],
        ...
        $.ajax({
            ...
            success: function(res) {
                if (res.code == "0") {
                    ...
                    $(".personal-center-mycollection").html(mycollectionAndDownLoadTemplate);
            ...
            success: function(res) {
                if (res && res.code == "0") {
                    var data = {
                        ...
                        scores: new Array(Number(res.data.score))
                    };
                    var evaluationDialogContent = template.compile(commentDialogContent)({
                        data: data
                    });
    ...
    function startTaskReceive(taskList) {
        ...
        var htmlTpl = template.compile(receiveCoupon)({
            data: data
        });
        var isVip = taskList.rewardType == 1 ? true : false;
        var vipClass = isVip ? "coupon-item-vip" : "";
        $("#dialog-box").dialog({
            html: $("#getcoupon-dialog").html().replace(/\$content/, htmlTpl).replace(/\$rewardType/, vipClass)
        }).open();
    }
    $(document).on("click", ".personal-center-mydownloads .evaluate-btn", function() {
    ...
    $(document).on("click", ".personal-center-dialog .file-rates .start", function() {
        ...
        if (isAppraise != 1) {
            ...
            //  $('.evaluation-dialog .evaluation-confirm').removeAttr("style");
    ...
    $("#dialog-box").on("click", ".close-btn", function() {
        if (isAppraise != 1) {
            getDownloadRecordList();
        }
        score = 0;
