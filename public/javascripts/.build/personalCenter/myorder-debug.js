define("dist/personalCenter/myorder-debug", [ "../application/method-debug", "../application/api-debug", "../application/urlConfig-debug", "../application/effect-debug", "../application/checkLogin-debug", "../application/login-debug", "../application/loginOperationLogic-debug", "../cmd-lib/jqueryMd5-debug", "../common/bindphone-debug", "../cmd-lib/myDialog-debug", "../cmd-lib/toast-debug", "../application/iframe/iframe-messenger-debug", "../application/iframe/messenger-debug", "../common/baidu-statistics-debug", "../common/loginType-debug", "../common/associateWords-debug", "./home-debug", "swiper-debug", "../common/recommendConfigInfo-debug", "../common/template/swiper_tmp-debug.html", "./template/homeRecentlySee-debug.html", "./template/vipPrivilegeList-debug.html", "./template/simplePagination-debug.html", "./template/myorder-debug.html" ], function(require) {
    ...
    $(".personal-center-myorder").click(".goods-name", function(event) {
        ...
        if (goodsName) {
            if (goodsType == "1") {
                ...
                params = "?fid=" + fid + "&ft=" + format + "&checkStatus=" + "10" + "&name=" + encodeURIComponent(encodeURIComponent(goodsName)) + "&ref=";
                method.compatibleIESkip("/pay/vip.html" + params, true);
            } else if (goodsType == "8") {
                params = "?fid=" + fid + "&ft=" + format + "&checkStatus=" + "13" + "&name=" + encodeURIComponent(encodeURIComponent(goodsName)) + "&ref=";
    ...
    function queryOrderlistByCondition(currentPage) {
        var orderStatus = method.getParam("myorderType") == "1" || method.getParam("myorderType") == "" ? [] : method.getParam("myorderType") == 0 ? [ 0, 1 ] : [ Number(method.getParam("myorderType")) ];
        $.ajax({
            headers: {
            ...
            success: function(res) {
                ...
                if (res.code == "0") {
                    ...
                    var myorderTemplate = template.compile(myorder)({
                        list: list || [],
                        myorderType: myorderType
                    });
                    $(".personal-center-myorder").html(myorderTemplate);
