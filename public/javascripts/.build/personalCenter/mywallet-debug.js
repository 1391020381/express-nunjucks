define("dist/personalCenter/mywallet-debug", [ "../cmd-lib/jquery.datepicker-debug", "./home-debug", "swiper-debug", "../common/recommendConfigInfo-debug", "../application/method-debug", "../common/template/swiper_tmp-debug.html", "../application/api-debug", "../application/urlConfig-debug", "./template/homeRecentlySee-debug.html", "./template/vipPrivilegeList-debug.html", "../application/effect-debug", "../application/checkLogin-debug", "../application/login-debug", "../application/loginOperationLogic-debug", "../cmd-lib/jqueryMd5-debug", "../common/bindphone-debug", "../cmd-lib/myDialog-debug", "../cmd-lib/toast-debug", "../application/iframe/iframe-messenger-debug", "../application/iframe/messenger-debug", "../common/baidu-statistics-debug", "../common/loginType-debug", "../common/associateWords-debug", "../cmd-lib/upload/Q-debug", "../cmd-lib/upload/Q.Uploader-debug", "./template/simplePagination-debug.html", "./template/mywallet-debug.html", "../cmd-lib/util-debug", "../common/area-debug", "../common/bankData-debug", "./dialog-debug" ], function(require) {
    ...
    var mywallet = require("./template/mywallet-debug.html");
    ...
    var bankData = require("../common/bankData-debug");
    ...
    function initCallback(data) {
        userInfo = data;
        getUserCentreInfo(function(res) {
            if (res.isAuth == "0") {
    ...
    function handleWithdrawalRecordData(res) {
        ...
        var mywalletTemplate = template.compile(mywallet)({
            list: list || [],
            mywalletType: mywalletType
        });
        $(".personal-center-mywallet").html(mywalletTemplate);
    ...
    $(document).on("change", ".mywallet .item-province", function() {
    ...
    $(document).on("click", ".mywallet .submit-btn", function() {
    ...
    $(document).on("input", ".withdrawal-application-dialog .amount", function() {
        // 查询个人提现扣税结算
        var reg = /^(([1-9][0-9]*)|(([0]\.\d{1,2}|[1-9][0-9]*\.\d{1,2})))$/;
        // 校验金额
        var withdrawPrice = Number($(this).val());
        if (!reg.test(Number(withdrawPrice))) {
            $.toast({
    ...
    $(document).on("click", ".withdrawal-application-dialog .full-withdrawal", function() {
        fullWithdrawal();
    });
    ...
    $(document).on("click", ".withdrawal-application-dialog .confirm-btn", function() {
        // 申请提现
        ...
        if (!withPrice && (errMsg = "请输入提现金额") || !reg.test(Number(withPrice)) && (errMsg = "请输入正确的金额(小数位只允许输入两位)") || financeAccountInfo.userTypeName == "机构" && !invoicePicUrl && (errMsg = "请上传发票图片")) {
            $.toast({
                text: errMsg,
                delay: 3e3
            });
    ...
    $(document).on("click", ".withdrawal-application-dialog .cancel-btn", function() {
        // 隐藏dialog
        closeRewardPop();
    });
    $(document).on("click", ".go2FinanceAccount-dialog .editFinanceAccount-cancel", function() {
        closeRewardPop();
    });
    $(document).on("click", ".go2FinanceAccount-dialog .editFinanceAccount-confirm", function() {
    ...
    $(document).on("click", ".balance-reflect", function() {
    ...
    $(document).on("click", ".survey-content .export-details-btn", function() {
    ...
    $(document).on("click", ".send-email-dialog .submit-btn", function() {
        // 导出
    ...
    $(document).on("change", ".receiving-bank .item-bank", function() {
    ...
    $(document).on("click", ".mywallet .survey-content-select .select-btn", function() {
