define("dist/detail/paging-debug", [ "../cmd-lib/toast-debug", "./template/img_box-debug.html", "./changeShowOverText-debug", "./download-debug", "../application/method-debug", "../cmd-lib/myDialog-debug", "../cmd-lib/loading-debug", "../cmd-lib/util-debug", "./common-debug", "../application/api-debug", "../application/urlConfig-debug", "./template/pay_btn_tmp-debug.html", "../common/loginType-debug", "../application/checkLogin-debug", "../application/login-debug", "../application/loginOperationLogic-debug", "../cmd-lib/jqueryMd5-debug", "../common/bindphone-debug", "../application/iframe/iframe-messenger-debug", "../application/iframe/messenger-debug", "../common/baidu-statistics-debug", "./changeDetailFooter-debug" ], function(require) {
    // var $ = require("$");
    require("../cmd-lib/toast-debug");
    var imgTmp = require("./template/img_box-debug.html");
    var changeText = require("./changeShowOverText-debug").changeText;
    var readMoreTextEvent = require("./changeShowOverText-debug").readMoreTextEvent;
    var initStyle = require("./changeDetailFooter-debug").initStyle;
    var loadMoreStyle = require("./changeDetailFooter-debug").loadMoreStyle;
    if (!window.pageConfig.imgUrl) return;
    // 启始页 默认的情况
    var cPage = Number(window.pageConfig.page.initReadPage);
    var restPage = 0;
    var imgTotalPage = Number(window.pageConfig.imgUrl.length);
    var totalPage = Number(window.pageConfig.params.totalPage);
    // 最大页数
    var ptype = window.pageConfig.params.g_fileExtension || "";
    var preRead = Number(window.pageConfig.page.preRead) || 50;
    // 预览页数
    var limitPage = Math.min(preRead, 50);
    // 最大限制阅读页数
    var initReadPage = Number(window.pageConfig.page.initReadPage);
    // 默认展示的页数
    ...
    var action = {
        ...
        drawing: function(currentPage) {
            ...
            // 拿到数据进行渲染
            var htmltpl = template.compile(imgTmp)({
                data: arr,
                ptype: ptype,
                copy: copy
            });
            if (ptype === "txt") {
                $(".font-detail-con").append(htmltpl);
            } else {
                $(".ppt-pic-con").append(htmltpl);
            }
            $("img.lazy").lazyload({
                effect: "fadeIn"
            });
        ...
        handleHtmlExpireUrl: function() {
            ...
            var sdate = Number(new Date(strDate));
            // 有效日期
    ...
    $(function() {
        ...
        if (hash && hash.split("#page")[1]) {
            ...
            if (hashArr.length) {
                var hashPage = Math.min(Number(hashArr[1]), imgTotalPage, preRead);
                if (hashPage <= 2) {
    ...
    function displayLoading(num) {
        ...
        var ltype = window.pageConfig.params.g_fileExtension || "";
        var supportSvg = window.pageConfig.supportSvg;
        var svgFlag = window.pageConfig.svgFlag;
        var copy = window.pageConfig.copy;
    ...
    function addItem(src, num) {
        var $item = "", padding = "";
        var addtype = window.pageConfig.params.g_fileExtension || "";
        var supportSvg = window.pageConfig.supportSvg;
        var svgFlag = window.pageConfig.svgFlag;
        var copy = window.pageConfig.copy;
        if (addtype === "txt") {
            ...
            $item += "<div class='detail-con third-style article-page source-link ' data-num='" + num + "'>" + "<div class='detail-inner article-main'>" + "<div class='data-detail " + padding + "'>" + "<embed src='" + src + "' width='100%' height='100%' type='image/svg+xml' pluginspage ='//www.adobe.com/svg/viewer/install/'/>" + (copy ? "" : "<article class='detail-holder'></article>") + "</div>" + "</div>" + "</div>";
        } else {
            if (addtype === "ppt") {
                padding = "ppt-format-style";
            } else {
    ...
    function addItemLoading(src, num) {
        var $item = "", padding = "";
        var loadtype = window.pageConfig.params.g_fileExtension || "";
        var supportSvg = window.pageConfig.supportSvg;
        var svgFlag = window.pageConfig.svgFlag;
        if (loadtype === "txt") {
