define("dist/detail/download-debug", [ "../application/method-debug", "../cmd-lib/myDialog-debug", "../cmd-lib/toast-debug", "../cmd-lib/loading-debug", "../cmd-lib/util-debug", "./common-debug", "../application/api-debug", "../application/urlConfig-debug", "./template/pay_btn_tmp-debug.html", "../common/loginType-debug", "../application/checkLogin-debug", "../application/login-debug", "../application/loginOperationLogic-debug", "../cmd-lib/jqueryMd5-debug", "../common/bindphone-debug", "../application/iframe/iframe-messenger-debug", "../application/iframe/messenger-debug", "../common/baidu-statistics-debug" ], function(require, exports, module) {
    ...
    // 文件id
    var tplAndroid = $("#tpl-down-android").html();
    ...
    var bouncedType = function(res) {
        ...
        switch (res.data.checkStatus) {
          // 下载
            ...
            method.delCookie("event_data_down", "/");
            if (res.data.score > 0) {
                expendScoreNum_var = Number(res.data.score);
            } else if (res.data.volume > 0) {
                expendNum_var = Number(res.data.volume);
            ...
            if (browserEnv === "IE" || browserEnv === "Edge") {
                ...
                var ffid = window.pageConfig.params.g_fileId;
                var fconsumeStatus = res.data.consumeStatus;
                var furl = "/node/f/downsucc.html?fid=" + ffid + "&consumeStatus=" + fconsumeStatus + "&url=" + encodeURIComponent(res.data.fileDownUrl);
                goNewTab(furl);
            } else {
                var sfid = window.pageConfig.params.g_fileId;
                var sconsumeStatus = res.data.consumeStatus;
                var surl = "/node/f/downsucc.html?fid=" + sfid + "&consumeStatus=" + sconsumeStatus + "&title=" + encodeURIComponent(file_title) + "&url=" + encodeURIComponent(res.data.fileDownUrl);
                goNewTab(surl);
            }
            break;

          case 8:
            var bfid = window.pageConfig.params.g_fileId;
            var format = window.pageConfig.params.file_format;
            var title = window.pageConfig.params.file_title;
            var params = "";
            var ref = utils.getPageRef(bfid);
            ...
            params = "?orderNo=" + bfid + "&checkStatus=" + res.data.checkStatus + "&referrer=" + document.referrer;
          ...
          case 10:
            var tfid = window.pageConfig.params.g_fileId;
            var showTips = 1;
            var turl = "";
