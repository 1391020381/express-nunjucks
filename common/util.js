                ...
                if (res.type == 1) { // 资料
                    ...
                    if (targetItem) {
                        if (targetItem.order === 4) {
                            // 追加字段-如果为办公站点
                            res.linkUrl = `${targetItem.pvalue}/${res.tprId}.html`;
                        } else {
                            // 追加字段-非办公站点
                            res.linkUrl = `${targetItem.desc}${targetItem.pvalue}/${res.tprId}.html`;
                        }
                    }
                }
            });
            ...
            if (item.type == 3 && item.tprId && item.expand && item.expand.templateCode && Array.isArray(dictionaryList)) { // 专题页
                ...
                if (targetItem) {
                    if (targetItem.order === 4) {
                        // 追加字段-如果为办公站点
                        temp = Object.assign({}, item, {
                            linkUrl: `${targetItem.pvalue}/${item.tprId}.html`
                        });
                    } else {
                        // 追加字段-非办公站点
                        temp = Object.assign({}, item, {
                            linkUrl: `${targetItem.desc}${targetItem.pvalue}/${item.tprId}.html`
                        });
                    }
                }
